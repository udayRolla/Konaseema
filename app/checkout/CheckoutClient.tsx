              ...
              {(cart.items || []).length === 0 ? (
                ...
                (cart.items || []).map((it: any) => (
                  <div key={it.id} className="flex justify-between mb-2">
                    <span>
                      {it.name} × {it.qty}
                      {it.weight ? <span className="opacity-70"> ({it.weight})</span> : null}
                    </span>
                    <span>{formatPrice(Number(it.qty) * Number(it.price))}</span>
                  </div>
                ))
              )}
              ...
              <div className="mt-6">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-3 rounded-2xl border border-gold bg-[#fffaf2]"
                  />
                  <button
                    type="button"
                    onClick={applyCoupon}
                    className="px-5 py-3 rounded-2xl border border-gold font-semibold"
                  >
                    Apply
                  </button>
                </div>
                {couponMsg && <div className="mt-2 text-sm">{couponMsg}</div>}
              </div>
            </section>
